...
class CategoryController extends Controller {
    ...
    public function check_find(Request $request) {
        ...
        $request->validate([
            'id' => 'nullable|numeric',
            'name' => "nullable|regex:$alphabetic|min:1|max:50",
                // por poner algo, esto es un check de find, no es necesario
        ]);
    }
...
//        $cat->save();
    ...
    public function find(Request $request) {
        ...
        $objCat = null; //initialize objCat as null
        ...
        try {
            ...
            if (isset($request->name)) {
                $objCat = Category::where('name', '=', $request->name)->first();
                return redirect()->to('/category/edit/' . $objCat->id);
            }
        ...
        if (is_null($objCat)) { // if it's null, it means both id and name inputs are empty
            $message = "You didn't introduced any id or name to look up";
        }
